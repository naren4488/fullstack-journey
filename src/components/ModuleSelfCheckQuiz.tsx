      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        {revealed ? (
          <span className="text-xs font-medium text-stone-600">
            Correct: <strong className="text-stone-900">{question.choices[question.correctIndex]}</strong>
          </span>
        ) : null}
      </div>
    ...
    <section className="rounded-[1.75rem] border border-stone-200/80 bg-white/80 p-6 shadow-[0_16px_40px_rgba(87,57,24,0.08)]">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-bold text-stone-950">{quiz.title}</h2>
          {quiz.blurb ? (
            <p className="mt-2 max-w-2xl text-sm leading-6 text-stone-600">{quiz.blurb}</p>
          ) : null}
        </div>
        <div className="rounded-full border border-stone-200 bg-stone-50 px-4 py-2 text-xs font-semibold text-stone-600">
